...
from utils.other_func import send_all_admin
...
# Выключение покупок
@dp.message_handler(IsAdmin(), text="🔴 Выключить Выдачу", state="*")
async def turn_off_buy(message: types.Message, state: FSMContext):
    await state.finish()
    update_settingsx(status_buy="False")
    await message.answer("<b>🔴 Выдача в боте были выключены.</b>",
                         reply_markup=get_settings_func())
    await send_all_admin(
        f"👤 Администратор <a href='tg://user?id={message.from_user.id}'>{(message.from_user.first_name)}</a>\n"
        "🔴 Выключил Выдачу в боте.", not_me=message.from_user.id)


# Включение покупок
@dp.message_handler(IsAdmin(), text="🟢 Включить Выдачу", state="*")
async def turn_on_buy(message: types.Message, state: FSMContext):
    await state.finish()
    update_settingsx(status_buy="True")
    await message.answer("<b>🟢 Выдача в боте были включены.</b>",
                         reply_markup=get_settings_func())
    await send_all_admin(
        f"👤 Администратор <a href='tg://user?id={message.from_user.id}'>{(message.from_user.first_name)}</a>\n"
        "🟢 Включил Выдачу в боте.", not_me=message.from_user.id)
...
async def get_text_for_change_faq(message: types.Message, state: FSMContext):
    ...
    if "{firstname}" in msg:
        msg = msg.replace("{firstname}", f"<b>{(message.from_user.first_name)}</b>")
    try:
        await state.finish()
