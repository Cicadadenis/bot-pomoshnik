...
async def category_remove_all(message: types.Message, state: FSMContext):
    ...
    await message.answer("<b>📜 Вы действительно хотите удалить все категории? ❌</b>\n"
                         "❗ Так же будут удалены все позиции и что в ней",
...
async def choice_category_for_edit_position(message: types.Message, state: FSMContext):
    ...
    if len(get_positions) >= 1:
        ...
        await message.answer("<b>❌ Отсутствуют позиции для добавления.</b>")


# Кнопка с изменением товаров
@dp.message_handler(IsAdmin(), text="🔐 Изменить 🖍", state="*")
async def open_edit_items(message: types.Message, state: FSMContext):
    await state.finish()
    await message.answer("<b>🔹 Получение всех Акаунтов и их позиций:</b> /getinfoitems\n"
                         "<b>🔸 Получение всех позиций:</b> /getposition\n"
                         "<b>🔹 Получение всех Акаунтов:</b> /getitems\n",
                         reply_markup=delete_item_inl)


# Кнопки с подтверждением удаления всех категорий
@dp.message_handler(IsAdmin(), text="🔐 Удалить ❌", state="*")
async def open_create_category(message: types.Message, state: FSMContext):
    await state.finish()
    await message.answer("<b>🔐 Вы действительно хотите удалить все ?</b> ❌\n",
...
async def category_create_input_name(message: types.Message, state: FSMContext):
    ...
    add_categoryx(category_id[0], f"☑️ {message.text}")
    await state.finish()
...
async def category_name_was_changed(message: types.Message, state: FSMContext):
    ...
    update_categoryx(category_id, category_name=f"☑️ {message.text}")
    await state.finish()
...
async def category_remove_all_confirm(call: CallbackQuery, state: FSMContext):
    ...
    clear_positionx()
    clear_itemx()
    await bot.edit_message_text("<b>☑ Вы успешно удалили все категории, позиции</b>",
                                call.from_user.id,
                                call.message.message_id)
...
async def position_input_name(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        data["here_input_position_name"] = f"🏷 {message.text}"
...
async def position_skip_get_image(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        position_price =  1   #data["here_input_position_price"]
...
async def delete_item(call: CallbackQuery, state: FSMContext):
    ...
    await bot.send_message(call.from_user.id,
                           "<b>🔐 Отправьте айди Акаунта для удаления</b>\n"
                           "❕ Если хотите удалить несколько Акаунтов, отправьте ID Акаунтов через запятую или пробел. Пример:\n"
...
async def input_item_id_for_delete(message: types.Message, state: FSMContext):
    ...
    await state.finish()
    await message.answer(f"<b>✅ Успешно удалённые Акаунты:</b>\n"
                         f"<code>▶ {remove_ids}</code>\n"
                         f"➖➖➖➖➖➖➖➖➖➖\n"
                         f"<b>❌ Ненайденные Акаунты:</b>\n"
...
async def confirm_clear_all_items(call: CallbackQuery, state: FSMContext):
    await bot.delete_message(call.from_user.id, call.message.message_id)
    delete_msg = await bot.send_message(call.from_user.id, "<b>⌛ Ждите, Акаунты удаляются...</b>")
    get_items = get_all_itemsx()

    clear_itemx()
    await bot.edit_message_text("<b>☑ Вы успешно удалили все Акаунты</b>",
                                call.from_user.id,
                                delete_msg.message_id)
...
async def cancel_remove_all_items(call: CallbackQuery, state: FSMContext):
    await bot.edit_message_text("<b>☑ Вы отменили удаление всех Акаунтов</b>",
                                call.from_user.id,
                                call.message.message_id)
